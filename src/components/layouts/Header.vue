 ...
 :class="changeColor || isOpen === true ? 'bg-bgcolor shadow-lg' : 'bg-transparent'">
  <div class="relative items-center justify-center mx-auto 2xl:w-8/12 sm:w-10/12 lg:flex">
    <RouterLink id="Home" aria-label="go to homepage" to="/">
      <img src="/logo.svg" alt="IBI Holding" width="241" height="76" center cover responsive loading="eager" class="aspect-3.17/1 max-md:ps-[8.333333%] max-sm:ps-6"/>
    </RouterLink>
    <!-- Desktop -->
    ...
    <!-- Mobile -->
    <button aria-label="open menu" class="absolute flex items-center justify-end w-8 h-8 md:right-0 right-[8.333333%] lg:hidden top-6" @click="toggleOpen">
    ...
    <Transition>
    <nav v-show="isOpen" class="w-full py-10 max-md:ps-[8.333333%] space-y-6 lg:hidden bg-bgcolor">
