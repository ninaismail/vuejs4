...
<div v-if="isOpen" class="fixed inset-0 z-[3] w-full h-screen" @click="isOpen = false"/>
 ...
 :class="changeColor || isOpen === true ? 'bg-bgcolor shadow-lg' : 'bg-transparent'">
  <div class="relative items-center justify-center mx-auto lg:w-10/12 lg:flex">
    ...
    <RouterLink id="go-to-contact-section" aria-label="go to contact section" to="/contact" class="text-nowrap lg:block hidden font-[800] text-2xl top-4 w-fit cursor-pointer py-2 px-5 transition-all duration-400 text-center rounded-[2px] text-offwhite bg-primary hover:brightness-125">Contact Us</RouterLink>
    <!-- Mobile -->
    <button aria-label="open menu" class="absolute flex items-center justify-center w-8 h-8 right-[8.333333%] lg:hidden top-6" @click="toggleOpen">
      ...
      <Transition>
      <h1 v-if="isOpen" class="text-4xl text-offwhite">&#10005;</h1>
      </Transition> 
    </button>
    <Transition>
